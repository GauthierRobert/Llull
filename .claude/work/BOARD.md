...
- **Batch 1 (DONE — all 4 lanes merged + pushed, 423 tests green):** L1 `U1` units (`set_units`, units/displayPrecision, `formatLength`; 29 cmds now) · L2 `V1` PBR/IBL/soft-shadows/tone-mapping · L3 `S4` advanced object snaps (perpendicular/tangent/extension/nearest + osnap tracking, +48 tests) · L4 `J1` MCP resources (`cad://document|scene|selection`). `cad-reviewer`: APPROVE on U1/V1/J1 (2 NITs, one applied); S4 review in flight. **Isolation caveat: J1 ran in the MAIN checkout (worktree didn't engage) — landed coherently, but watch for it.**
