...
- **Batch 1 (LAUNCHED — 4 parallel lanes, disjoint files, no cross-deps):** L1 `U1` units · L2 `V1` PBR/lighting · L3 `S4` advanced object snaps (re-land) · L4 `J1` document-as-MCP-resource.
- **Next batches:** B2 = `AI3` named/tagged + find_entities · `V3` status bar · `U4` 2D grid/scale · `AI2` render_view. B3 = `M1` measure · `V2` command palette · `S1↔VS1` ellipse/spline · export endpoints. B4 = `Q1` parameters · `M2` HUD · 2D modify · `AI4` check_model.
...
- `[WIP]` **U1** Units system: add `units` (`'mm'|'cm'|'m'|'in'|'ft'`, default `mm`) + display precision to `CadDocument`; `set_units` command; thread through persistence + measure summaries. _Lane 1. deps: —. Acceptance: round-trips through save/load; summaries report values with unit suffix._
...
- `[WIP]` **V1** Material/lighting upgrade: PBR materials, environment map (drei `Environment`), soft contact shadows / AO, tone mapping. _Lane 2. deps: —._
...
- `[WIP]` **J1** Expose the document as an MCP **resource** (read-only): list entities, read full document / selection. _deps: G2._
