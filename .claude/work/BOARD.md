...
- **Last updated:** 2026-05-25 — **BATCHES 2 + 3 DONE (603 tests green, 27 files).** Wave 3 backlog appended (KI/NF/EN). Committed to branch `feat/wave3-batches-2-3`. Batch 2: `P1` (on-demand render), `AI3` (name/tags + `find_entities` + `set_entity_name`), `V3` (status bar + dark/light theme), `U4` (2D adaptive grid + scale bar + float-origin pan/zoom), `KI2` (`CommandResult.data` → MCP `structuredContent`). Batch 3: `M1` (7 measure/query tools), `V2` (command palette + shortcuts + view presets), `EN2` (MCP `prompts/`). **38 commands.** Every task `cad-reviewer`-passed; reviewers caught + fixed **2 real blocking bugs** (indexed-mesh volume formula; prompt-plan param names/sequencing). **Next: Batch 4** = `Q1` parameters · `S1↔VS1` ellipse/spline · `L1↔L2` layers · `AI4` check_model · `M2` measurement HUD (uses M1). Lane 3 unblocks once `S1`/`S2` land.
...
- **Next batches:** B4 = `Q1` parameters (uses W1) · `S1↔VS1` ellipse/spline · `L1↔L2` layers · `M2` measurement HUD (uses M1) · `AI4` check_model. B5 = `N1↔VN1` cone/torus/wedge/pyramid · `T1↔VT1` text · `X1` STL export · Wave 3 `EN1` MCP tool annotations / `KI1` MCP session model / `EN3` spatial find_entities (uses AI3). `AI2` render_view deferred pending a headless-GL spike (Windows native-deps risk).
...
- `[TODO]` **KI1** MCP session & document model: replace the single module-level `workingDoc` in `server/src/mcp.ts` with **session-keyed** documents (SDK stateful transport supplies a session id) + a bridge to load/snapshot the UI document into an MCP session and read it back. _Lane 4. deps: J1. Acceptance: two concurrent MCP clients don't clobber each other; an agent can edit a UI-exported doc; UI↔MCP exchange documented._
